...
# zjx, change input, add class_name
def clean_data(gt_anno, dt_anno, current_class, difficulty, class_name):
    CLASS_NAMES = class_name # zjx, change CLASS_NAMES to input class_name
...
# zjx, change input, add class_name
def _prepare_data(gt_annos, dt_annos, current_class, difficulty, class_name):
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
    ...
    for i in range(len(gt_annos)):
        rets = clean_data(gt_annos[i], dt_annos[i], current_class, difficulty, class_name) # zjx, add class_name
...
def eval_class(gt_annos,
    ...
    Args:
        ...
        num_parts: int. a parameter for fast calculate algorithm
        class_name: str. current_classes corresponding class_name
    Returns:
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            rets = _prepare_data(gt_annos, dt_annos, current_class, difficulty, class_name) # zjx, add class_name
...
def do_eval(gt_annos,
    ...
    ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 0,
                     min_overlaps, compute_aos, class_name=class_name) # zjx, add class_name
    ...
    ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 1,
                     min_overlaps, class_name=class_name) # zjx, add class_name
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    # zjx, change class name
    class_to_name = {}
    for num in range(len(current_classes)):
        class_to_name[num] = current_classes[num]
    # class_to_name = {
    #     0: 'Car',
    #     1: 'Pedestrian',
    #     2: 'Cyclist',
    ...
    #     5: 'Truck'
    # }
    # TODO: change overlap threshold
    overlap_0_7 = np.array(0.7 * np.ones((3, len(class_to_name))))
    overlap_0_5 = np.array(0.5 * np.ones((3, len(class_to_name))))
    ...
    min_overlaps = np.stack([overlap_0_7, overlap_0_5], axis=0)  # [2, 3, N]

    name_to_class = {v: n for n, v in class_to_name.items()}
    if not isinstance(current_classes, (list, tuple)):
        current_classes = [current_classes]
    current_classes_int = []
    current_classes_name = current_classes # zjx, save class name
    ...
    # zjx, change do_eval input, add class_name
    mAPbbox, mAPbev, mAP3d, mAPaos, mAPbbox_R40, mAPbev_R40, mAP3d_R40, mAPaos_R40 = do_eval(
        gt_annos, dt_annos, current_classes, min_overlaps, compute_aos, PR_detail_dict=PR_detail_dict, class_name=current_classes_name)
